...
public class AfternoonShiftRecycler extends RecyclerView.Adapter<AfternoonShiftRecycler.ViewHolder>
{
    private final Context context;
    public static List<Employee> afternoonShifts = new ArrayList<>();
    private boolean isSwiped = false;

    public AfternoonShiftRecycler(Context context) {
        this.context = context;
    }

    @NonNull
    @Override
    public AfternoonShiftRecycler.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.employee_item_shift, parent, false);
        return new AfternoonShiftRecycler.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull AfternoonShiftRecycler.ViewHolder holder, int position) {
        Employee employee = afternoonShifts.get(position);
    ...
    public int getItemCount() {
        return afternoonShifts.size();
    }

    private void resetTranslation(@NonNull AfternoonShiftRecycler.ViewHolder holder) {
    ...
    private void removeFromShift(Employee employee) {
        MainActivity.database.schedulingTable.deleteFromShift(employee, "aftr_");
        afternoonShifts.remove(employee);
        notifyDataSetChanged();
    }

    private void undoRemoval(Employee employee) {
        MainActivity.database.schedulingTable.storeDateData(employee);
        afternoonShifts.add(employee);
        notifyDataSetChanged();
    }
