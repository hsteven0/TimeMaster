...
public class MorningShiftRecycler extends RecyclerView.Adapter<MorningShiftRecycler.ViewHolder>
{
    private final Context context;
    public static List<Employee> morningShifts = new ArrayList<>();
    private boolean isSwiped = false;

    public MorningShiftRecycler(Context context) {
        this.context = context;
    }

    @NonNull
    @Override
    public MorningShiftRecycler.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.employee_item_shift, parent, false);
        return new MorningShiftRecycler.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull MorningShiftRecycler.ViewHolder holder, int position) {
        Employee employee = MorningShiftRecycler.morningShifts.get(position);
    ...
    private void resetTranslation(@NonNull MorningShiftRecycler.ViewHolder holder) {
    ...
    private void removeFromShift(Employee employee) {
        MainActivity.database.schedulingTable.deleteFromShift(employee, "morn_");
        morningShifts.remove(employee);
        notifyDataSetChanged();
    }

    private void undoRemoval(Employee employee) {
        MainActivity.database.schedulingTable.storeDateData(employee);
        morningShifts.add(employee);
        notifyDataSetChanged();
    }

    @Override
    public int getItemCount() {
        return MorningShiftRecycler.morningShifts.size();
    }
